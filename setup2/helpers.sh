...
function wait_for_elasticsearch {
	...
	local -a args=( '-s' '-D-' '-m15' '-w' '%{http_code}' 'https://elasticsearch2:9200/'
		'--resolve' "elasticsearch2:9200:${elasticsearch_host}" '--cacert' "$es_ca_cert"
		)
...
function check_user_exists {
	...
	local -a args=( '-s' '-D-' '-m15' '-w' '%{http_code}'
		"https://elasticsearch2:9200/_security/user/${username}"
		'--resolve' "elasticsearch2:9200:${elasticsearch_host}" '--cacert' "$es_ca_cert"
		)
...
function set_user_password {
	...
	local -a args=( '-s' '-D-' '-m15' '-w' '%{http_code}'
		"https://elasticsearch2:9200/_security/user/${username}/_password"
		'--resolve' "elasticsearch2:9200:${elasticsearch_host}" '--cacert' "$es_ca_cert"
		'-X' 'POST'
...
function ensure_role {
	...
	local -a args=( '-s' '-D-' '-m15' '-w' '%{http_code}'
		"https://elasticsearch2:9200/_security/role/${name}"
		'--resolve' "elasticsearch2:9200:${elasticsearch_host}" '--cacert' "$es_ca_cert"
		'-X' 'POST'
		'-H' 'Content-Type: application/json'
		'-d' "$body"
